...
const hasValueColumn = computed(() => {
  ...
  return [
    ...
    'delivery_orders',
    'fuel_volume',
    'invoice_value', 'paid_value', 'outstanding_value', 'overdue_value',
    'sales_value'
  ].includes(key)
})
...
const filteredSubtotal = computed(() => {
  ...
  return filteredItems.value.reduce((sum, item) => sum + (Number(item.value) || 0), 0)
})
...
const statusColors: Record<string, Record<string, 'neutral' | 'success' | 'warning' | 'error' | 'info'>> = {
  offering_letter: { created: 'info', under_revision: 'warning', po_received: 'success' },
  ...
  user: { admin: 'neutral', marketing: 'info', operations: 'warning', finance: 'success' }
}

function statusBadge(status: string, group: string) {
  const map = statusColors[group] || {}
  const color = map[status] ?? 'neutral'
  return h(UBadge, { variant: 'subtle', color }, () => status?.replace(/_/g, ' ') ?? '-')
}
...
const columns = computed((): TableColumn<any>[] => {
  ...
  if (key === 'customers') return [
    ...
    { accessorKey: 'created_at', header: 'Terdaftar', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]
  ...
  if (key === 'users') return [
    ...
    { accessorKey: 'created_at', header: 'Dibuat', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]

  // ── Offering Letters ─────────────────────────────────────────
  if (key === 'offering_letters') return [
    { accessorKey: 'title', header: 'Nomor SP' },
    ...
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ row }) => statusBadge(row.getValue('status') ?? '', 'offering_letter')
    },
    { accessorKey: 'created_at', header: 'Dibuat', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]

  // ── PO Customer & PO Supplier ────────────────────────────────
  if (key === 'customer_purchase_orders' || key === 'supplier_purchase_orders') return [
    { accessorKey: 'title', header: 'Nomor PO' },
    { accessorKey: 'subtitle', header: key === 'customer_purchase_orders' ? 'Customer' : 'Supplier' },
    {
      accessorKey: 'value',
      header: 'Nilai PO',
      cell: ({ row }) => formatCurrency(row.getValue('value') ?? 0)
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ row }) => statusBadge(row.getValue('status') ?? '', 'purchase_order')
    },
    { accessorKey: 'created_at', header: 'Dibuat', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]

  // ── Delivery Orders & Fuel Volume ────────────────────────────
  if (key === 'delivery_orders' || key === 'fuel_volume') return [
    ...
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ row }) => statusBadge(row.getValue('status') ?? '', 'delivery_order')
    },
    { accessorKey: 'created_at', header: 'Dibuat', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]

  // ── Invoice Value / Paid / Outstanding / Overdue ─────────────
  if (['invoice_value', 'paid_value', 'outstanding_value', 'overdue_value'].includes(key)) return [
  ...
  if (key === 'sales_value') return [
    ...
    { accessorKey: 'created_at', header: 'Tanggal', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]

  // ── Notifikasi belum dibaca ───────────────────────────────────
  if (key === 'unread_notifications') return [
    { accessorKey: 'title', header: 'Judul' },
    { accessorKey: 'subtitle', header: 'Pesan' },
    {
      accessorKey: 'status',
      header: 'Tipe',
      cell: ({ row }) => statusBadge(row.getValue('status') ?? '', 'notification')
    },
    { accessorKey: 'created_at', header: 'Waktu', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]
  ...
  if (key === 'uploads') return [
    ...
    { accessorKey: 'subtitle', header: 'Keterangan' },
    {
      accessorKey: 'status',
      header: 'Tipe Dokumen',
      cell: ({ row }) => h(UBadge, { variant: 'soft', color: 'neutral' }, () => row.getValue('status') ?? '-')
    },
    {
      accessorKey: 'value',
      header: 'Ukuran (bytes)',
      cell: ({ row }) => row.getValue('value') ? formatNumber(row.getValue('value')) : '-'
    },
    { accessorKey: 'created_at', header: 'Diupload', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]
  ...
  return [
    ...
    { accessorKey: 'subtitle', header: 'Keterangan' },
    {
      ...
      cell: ({ row }) => row.getValue('status')
        ? h(UBadge, { variant: 'subtle', color: 'neutral' }, () => row.getValue('status'))
        : '-'
    },
    { accessorKey: 'created_at', header: 'Dibuat', cell: ({ row }) => row.getValue('created_at') ? formatDate(row.getValue('created_at')) : '-' }
  ]
...
const searchPlaceholder = computed(() => {
  ...
  if (key === 'customer_purchase_orders' || key === 'supplier_purchase_orders') return 'Cari nomor PO...'
  if (key === 'delivery_orders' || key === 'fuel_volume') return 'Cari nomor DO atau transportir...'
  if (['invoice_value', 'paid_value', 'outstanding_value', 'overdue_value'].includes(key)) return 'Cari nomor invoice atau customer...'
  ...
  >
    <template #title>
      <div class="flex items-center gap-2">
        <UIcon v-if="metric?.key" name="i-lucide-table-2" class="size-4 text-primary" />
      ...
      <div v-if="metric" class="space-y-4">
        ...
        <div class="rounded-lg bg-muted/50 p-4 space-y-1">
          <div class="flex items-center justify-between">
            <p class="text-sm font-medium text-highlighted">{{ metric.title }}</p>
          ...
          <p v-if="data?.total_value && data.total_value !== metric.value" class="text-xs text-muted pt-1 border-t border-default mt-2">
            Total pada halaman ini:
            <span class="font-medium text-highlighted">{{ formatSummaryValue(data.total_value) }}</span>
          </p>
        </div>

        <!-- Error -->
        <UAlert v-if="error" color="error" title="Gagal memuat rincian" :description="error.message" />
        ...
        <template v-else>
          ...
          >
            <span class="text-muted">
              Subtotal {{ search ? `(${filteredItems.length} hasil pencarian)` : `(${filteredItems.length} record)` }}
            </span>
          ...
          <div v-if="(data?.total || 0) > 50" class="flex items-center justify-between border-t border-default pt-3">
            <p class="text-xs text-muted">{{ data?.total }} total record</p>
            <UPagination v-model:page="page" :total="data.total" :items-per-page="50" />
          </div>
        </template>
      </div>
    </template>
