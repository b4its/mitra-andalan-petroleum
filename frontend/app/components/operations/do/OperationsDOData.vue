...
async function openEdit(row: any) {
  ...
  try {
    ...
    lengkapiForm.do_number = d.doInformation?.doNumber || row.deliveryOrderNumber || ''
    lengkapiForm.do_date = d.doInformation?.doDateCreated || new Date().toISOString().split('T')[0] || ''
    ...
    lengkapiForm.receiver_date = d.receiverDateReceived || new Date().toISOString().split('T')[0] || ''
    ...
    lengkapiForm.transport_number = d.transportInformation?.transportNumber || ''
    ...
    lengkapiForm.departure_time = d.transportInformation?.timeInformation?.departureTime || ''
    lengkapiForm.arrival_time = d.transportInformation?.timeInformation?.arrivalTime || ''
    lengkapiForm.depot_arrival_time = d.transportInformation?.timeInformation?.depotArrivalTime || ''
    lengkapiForm.unloading_time = d.transportInformation?.timeInformation?.unloadingTime || ''
    lengkapiForm.company_coordinator = d.companyCoordinator || 'Admin'
    lengkapiForm.distribution_admin = d.distributionAdmin || user.value?.name || ''
    lengkapiForm.receiver_sign = d.receiver || ''
    lengkapiForm.driver_sign = d.driver || ''

    lengkapiOpen.value = true
  } catch {
    toast.add({ title: 'Error', description: 'Gagal memuat data DO.', color: 'error' })
  } finally {
...
async function submitLengkapi() {
  ...
  if (!lengkapiForm.do_number || !lengkapiForm.transport_name || !lengkapiForm.fuel_qty) {
    toast.add({ title: 'Validasi', description: 'Nomor DO, Transportir, dan Volume wajib diisi.', color: 'warning' })
    return
  }
  ...
  try {
    ...
    const details = {
      ...
      doInformation: {
        ...
        poCustomerNumber: { purchaseOrderNumber: lengkapiTarget.value.purchaseOrderNumber || '' },
    ...
    toast.add({ title: 'Berhasil', description: 'Data DO berhasil dilengkapi. Surat siap dirender.', color: 'success' })
    lengkapiOpen.value = false
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal menyimpan data.', color: 'error' })
  } finally {
...
function openReadyOrder(row: any) { readyOrderTarget.value = row; readyOrderOpen.value = true }
...
async function confirmReadyOrder() {
  ...
  try {
    ...
    toast.add({ title: 'Berhasil', description: 'Pengantaran telah disiapkan.', color: 'success' })
    readyOrderOpen.value = false
    readyOrderTarget.value = null
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Gagal', description: err.message || 'Gagal menyiapkan pengantaran.', color: 'error' })
  } finally {
...
function openSelesaiDikirim(row: any) { selesaiDikirimTarget.value = row; selesaiDikirimOpen.value = true }
...
async function confirmSelesaiDikirim() {
  ...
  try {
    await post(`/delivery-orders/${selesaiDikirimTarget.value.id}/selesai-dikirim`, {})
    toast.add({ title: 'Berhasil', description: 'Pengiriman ditandai selesai.', color: 'success' })
    selesaiDikirimOpen.value = false
    selesaiDikirimTarget.value = null
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Gagal', description: err.message || 'Gagal menandai selesai dikirim.', color: 'error' })
  } finally {
...
async function updateDoStatus(doId: string) {
  ...
  try {
    ...
    toast.add({ title: 'Berhasil', description: 'Status DO diperbarui.', color: 'success' })
  } catch {
    toast.add({ title: 'Gagal', description: 'Gagal memperbarui status DO.', color: 'error' })
  } finally {
    loading.value = false
    refresh()
  }
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const colorMap: Record<string, 'info' | 'success' | 'warning'> = {
        created: 'info', draft: 'warning', document_returned: 'success'
      }
      const labelMap: Record<string, string> = {
        created: 'Dibuat', draft: 'Draft', document_returned: 'Dokumen Kembali'
      }
      return h(UBadge, { variant: 'soft', color: colorMap[s] ?? 'neutral' }, () => labelMap[s] ?? s)
    }
  },
    ...
    >
      <template #actions-cell="{ row }">
        <div class="flex flex-wrap gap-1.5">
          ...
          <UButton
            v-if="row.original.statusRilisDana && !row.original.statusReadyOrder"
            size="xs" color="info" variant="soft"
            icon="i-lucide-package"
            :disabled="!row.original.detailsLengkap"
            :title="!row.original.detailsLengkap ? 'Lengkapi data DO terlebih dahulu' : ''"
          ...
          <UButton
            v-if="row.original.statusReadyOrder && !row.original.statusSelesaiDikirim"
          ...
          <!-- Tandai Dokumen Kembali -->
          <UButton
            v-if="row.original.status === 'created' || row.original.status === 'draft'"
            :loading="loading"
            size="xs" color="warning" variant="soft"
  ...
  <UModal v-model:open="readyOrderOpen" :ui="{ content: 'max-w-md' }">
    ...
    <template #body>
      <p class="text-sm text-muted">
        ...
        <span class="font-semibold text-highlighted">{{ readyOrderTarget?.deliveryOrderNumber }}</span>
        sudah disiapkan dan siap dikirim?
      </p>
      <p class="mt-2 text-xs text-dimmed">Waktu penyiapan akan dicatat (WITA).</p>
    </template>
    <template #footer>
      <div class="flex justify-end gap-2">
        <UButton color="neutral" variant="ghost" @click="readyOrderOpen = false">Batal</UButton>
        <UButton color="info" :loading="readyOrderLoading" icon="i-lucide-check" @click="confirmReadyOrder">
          Ya, Sudah Siap
        </UButton>
      </div>
    </template>
  ...
  <UModal v-model:open="selesaiDikirimOpen" :ui="{ content: 'max-w-md' }">
    ...
    <template #body>
      <p class="text-sm text-muted">
        ...
        <span class="font-semibold text-highlighted">{{ selesaiDikirimTarget?.deliveryOrderNumber }}</span>
        sudah selesai?
      </p>
      <p class="mt-2 text-xs text-dimmed">
        Setelah dikonfirmasi, Finance dapat melunasi ongkir. Waktu akan dicatat (WITA).
      </p>
    </template>
    <template #footer>
      <div class="flex justify-end gap-2">
        <UButton color="neutral" variant="ghost" @click="selesaiDikirimOpen = false">Batal</UButton>
        <UButton color="success" :loading="selesaiDikirimLoading" icon="i-lucide-check" @click="confirmSelesaiDikirim">
          Ya, Selesai Dikirim
        </UButton>
      </div>
    </template>
  ...
  <UModal v-model:open="lengkapiOpen" :ui="{ content: 'max-w-2xl' }">
    <template #title>
      <div class="flex items-center gap-2">
        ...
        {{ lengkapiTarget?.detailsLengkap ? 'Edit Data Delivery Order' : 'Lengkapi Data Delivery Order' }}
      </div>
    </template>

    <template #body>
      <div v-if="!lengkapiTarget?.detailsLengkap" class="mb-4 rounded-lg bg-warning/10 border border-warning/30 px-4 py-3 text-sm">
        <p class="font-medium text-warning">Data DO belum lengkap untuk dicetak.</p>
        <p class="text-xs mt-1 text-muted">Isi data berikut sesuai surat DO yang akan dicetak.</p>
      </div>
      ...
      <div class="space-y-5 text-sm">
        ...
        <!-- Header DO -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Header Surat</p>
          <div class="grid grid-cols-3 gap-3">
            <div>
              <label class="block text-xs text-muted mb-1">Nomor DO <span class="text-error">*</span></label>
              <UInput v-model="lengkapiForm.do_number" placeholder="0000/DO/MAP/I/0000" size="sm" />
            </div>
            <div>
            ...
            <div>
              ...
              <UInput v-model="lengkapiForm.so_number" placeholder="Opsional" size="sm" />
            </div>
          </div>
        </div>
        ...
        <!-- Penerima / Customer -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Penerima (Customer)</p>
          <div class="grid grid-cols-2 gap-3">
            <div class="col-span-2">
              <label class="block text-xs text-muted mb-1">Alamat Customer</label>
              <UInput v-model="lengkapiForm.customer_address" placeholder="Alamat customer" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Penerima BBM (Nama)</label>
              <UInput v-model="lengkapiForm.receiver_name" placeholder="Nama penerima" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">HP Penerima</label>
              <UInput v-model="lengkapiForm.receiver_phone" placeholder="08xxxxxxxxxx" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Tanggal Terima</label>
              <UInput v-model="lengkapiForm.receiver_date" type="date" size="sm" />
            </div>
          </div>
        </div>
        ...
        <!-- Transportir / Agen -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Agen / Transportir</p>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs text-muted mb-1">Nama Transportir <span class="text-error">*</span></label>
              <UInput v-model="lengkapiForm.transport_name" placeholder="PT. Transport Logistik" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">ID Transportir</label>
              <UInput v-model="lengkapiForm.transport_id" placeholder="ID / nama lengkap" size="sm" />
            </div>
            <div class="col-span-2">
              <label class="block text-xs text-muted mb-1">Alamat Transportir</label>
              <UInput v-model="lengkapiForm.transport_address" placeholder="Alamat transportir" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Nama Driver</label>
              <UInput v-model="lengkapiForm.driver_name" placeholder="Nama driver" size="sm" />
            </div>
            <div>
        ...
        <!-- Produk & Volume -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Produk & Volume</p>
          <div class="grid grid-cols-3 gap-3">
            <div>
              <label class="block text-xs text-muted mb-1">Nama Produk <span class="text-error">*</span></label>
              <UInput v-model="lengkapiForm.product_name" placeholder="Bio Diesel" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Volume (Liter) <span class="text-error">*</span></label>
              <UInput v-model.number="lengkapiForm.fuel_qty" type="number" placeholder="5000" size="sm" />
            </div>
            <div>
            ...
            <div>
              ...
              <UInput v-model="lengkapiForm.top_seal" placeholder="No. segel atas" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Segel Bawah</label>
              <UInput v-model="lengkapiForm.bottom_seal" placeholder="No. segel bawah" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Temperatur</label>
              <UInput v-model.number="lengkapiForm.temperature" type="number" placeholder="0" size="sm" />
            </div>
          </div>
        </div>
        ...
        <!-- Kendaraan & Waktu -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Kendaraan & Waktu</p>
          <div class="grid grid-cols-3 gap-3">
            <div>
              <label class="block text-xs text-muted mb-1">No. Kendaraan</label>
              <UInput v-model="lengkapiForm.transport_number" placeholder="KT 1234 AB" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Jenis Kendaraan</label>
              <UInput v-model="lengkapiForm.transport_type" placeholder="Truk Tangki" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">SG Meter</label>
              <UInput v-model="lengkapiForm.sg_meter" placeholder="0.000" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Km. Awal</label>
              <UInput v-model="lengkapiForm.start_km" placeholder="0" size="sm" />
            </div>
            <div>
            ...
            <div>
              ...
              <UInput v-model="lengkapiForm.departure_time" type="time" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Jam Tiba</label>
              <UInput v-model="lengkapiForm.arrival_time" type="time" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Jam Tiba di Depo</label>
              <UInput v-model="lengkapiForm.depot_arrival_time" type="time" size="sm" />
            </div>
            <div>
              <label class="block text-xs text-muted mb-1">Jam Mulai Bongkar</label>
              <UInput v-model="lengkapiForm.unloading_time" type="time" size="sm" />
            </div>
          </div>
        </div>
        ...
        <!-- Footer / Tanda Tangan -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Footer Surat</p>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs text-muted mb-1">Koordinator MAP</label>
              <UInput v-model="lengkapiForm.company_coordinator" placeholder="Admin" size="sm" />
            </div>
            <div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <UButton color="neutral" variant="ghost" @click="lengkapiOpen = false">Batal</UButton>
        <UButton
          color="primary"
          icon="i-lucide-save"
        ...
        >
          {{ lengkapiTarget?.detailsLengkap ? 'Simpan Perubahan' : 'Simpan & Aktifkan Surat' }}
        </UButton>
      </div>
    </template>
  </UModal>
