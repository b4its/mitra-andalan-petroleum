...
<template>
  ...
  >
    ...
    <template #body>
      <div class="mb-4 rounded-lg bg-warning/10 border border-warning/30 px-4 py-3 text-sm text-warning">
        <p class="font-medium">Data DO belum lengkap untuk dicetak.</p>
        <p class="text-xs mt-1 text-muted">Isi data berikut agar surat DO dapat dirender.</p>
      </div>

      <UForm :schema="schema" :state="form" class="space-y-4" @submit="onSubmit">
        <div class="grid grid-cols-2 gap-4">
          <UFormField name="do_number" label="Nomor DO" required>
            <UInput v-model="form.do_number" placeholder="0000/DO/MAP/I/0000" class="w-full" />
          </UFormField>
        ...
        <div class="grid grid-cols-2 gap-4">
          <UFormField name="transport_name" label="Nama Transportir" required>
            <UInput v-model="form.transport_name" placeholder="PT. Transport Logistik" class="w-full" />
          </UFormField>
          <UFormField name="transport_number" label="Nomor Kendaraan">
            <UInput v-model="form.transport_number" placeholder="KT 1234 AB" class="w-full" />
          </UFormField>
        </div>

        <div class="grid grid-cols-2 gap-4">
          <UFormField name="product_name" label="Nama Produk" required>
            <UInput v-model="form.product_name" placeholder="Bio Diesel" class="w-full" />
          </UFormField>
          <UFormField name="fuel_qty" label="Volume (Liter)" required>
            <UInput v-model.number="form.fuel_qty" type="number" placeholder="5000" class="w-full" />
          </UFormField>
        </div>

        <div class="grid grid-cols-2 gap-4">
          <UFormField name="driver_name" label="Nama Driver">
            <UInput v-model="form.driver_name" placeholder="Nama driver" class="w-full" />
          </UFormField>
