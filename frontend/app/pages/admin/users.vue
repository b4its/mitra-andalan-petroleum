...
// ── Data fetch ────────────────────────────────────────────────
const { data: users, pending, refresh } = await useAsyncData<User[]>(
  'admin-users',
  () => get<User[]>('/profiles'),
  { default: () => [], lazy: true }
)
...
const columns: TableColumn<User>[] = [
  ...
  { accessorKey: 'email', header: 'Email' },
  {
    ...
    cell: ({ row }) => {
      ...
      return h(UBadge, { variant: 'subtle', color: roleColor[r] ?? 'neutral', class: 'capitalize' }, () => r)
    }
  },
...
async function onSubmitAdd(event: FormSubmitEvent<AddSchema>) {
  ...
  try {
    ...
    toast.add({ title: 'Berhasil', description: 'Pengguna baru berhasil ditambahkan.', color: 'success' })
    modalOpen.value = false
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal menambahkan pengguna.', color: 'error' })
  } finally {
    saving.value = false
  }
}
...
async function onSubmitEdit(event: FormSubmitEvent<EditSchema>) {
  ...
  try {
    ...
    toast.add({ title: 'Berhasil', description: 'Data pengguna berhasil diperbarui.', color: 'success' })
    modalOpen.value = false
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal memperbarui pengguna.', color: 'error' })
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  try {
    ...
    const res = await fetch(`/api/v1/profiles/${deleteTarget.value.id}`, { method: 'DELETE' })
    if (!res.ok) {
      const err = await res.json().catch(() => ({ detail: res.statusText }))
      throw new Error(err.detail || `Error ${res.status}`)
    }
    toast.add({ title: 'Berhasil', description: `Pengguna ${deleteTarget.value.name} berhasil dihapus.`, color: 'success' })
    deleteOpen.value = false
    deleteTarget.value = null
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal menghapus pengguna.', color: 'error' })
  } finally {
...
const modalTitle = computed(() => {
  ...
  if (modalMode.value === 'edit') return `Edit Pengguna — ${selectedUser.value?.name ?? ''}`
    ...
    <template #body>
      ...
      <div v-if="modalMode === 'view' && selectedUser" class="space-y-4">
        <div class="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <UBadge :color="roleColor[selectedUser.role] ?? 'neutral'" variant="subtle" class="capitalize">
              {{ selectedUser.role }}
            </UBadge>
          </div>
          <div>
            <p class="text-xs text-muted uppercase tracking-wide mb-1">ID</p>
            <p class="font-mono text-xs text-muted truncate">{{ selectedUser.id }}</p>
          </div>
        </div>
      </div>
      ...
      >
        <UFormField name="name" label="Nama" required>
          <UInput v-model="formState.name" placeholder="Nama lengkap" autocomplete="off" />
        </UFormField>
