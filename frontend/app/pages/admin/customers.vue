...
// ── Data fetch ────────────────────────────────────────────────
const { data: customers, pending, refresh } = await useAsyncData<Customer[]>(
  'admin-customers',
...
async function onSubmitAdd(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    toast.add({ title: 'Berhasil', description: 'Customer baru berhasil ditambahkan.', color: 'success' })
    modalOpen.value = false
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal menambahkan customer.', color: 'error' })
  } finally {
    saving.value = false
  }
}
...
async function onSubmitEdit(event: FormSubmitEvent<Schema>) {
  ...
  saving.value = true
  try {
    await put<Customer, Schema>(`/customers/${selectedCustomer.value.id}`, event.data)
    toast.add({ title: 'Berhasil', description: 'Data customer berhasil diperbarui.', color: 'success' })
    modalOpen.value = false
    refresh()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Gagal memperbarui customer.', color: 'error' })
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    const res = await fetch(`/api/v1/customers/${deleteTarget.value.id}`, { method: 'DELETE' })
    if (!res.ok) {
    ...
    toast.add({ title: 'Error', description: err.message || 'Gagal menghapus customer.', color: 'error' })
  } finally {
...
const modalTitle = computed(() => {
  ...
  if (modalMode.value === 'edit') return `Edit Customer — ${selectedCustomer.value?.name ?? ''}`
    ...
    <template #body>
      ...
      <div v-if="modalMode === 'view' && selectedCustomer" class="space-y-4">
        <div class="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p class="font-mono text-xs text-muted truncate">{{ selectedCustomer.id }}</p>
          </div>
      ...
      >
        <UFormField name="name" label="Nama" required>
          <UInput v-model="formState.name" placeholder="Nama customer" autocomplete="off" />
        </UFormField>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        <UButton
          ...
          color="primary"
          :loading="saving"
          :form="modalMode === 'add' ? 'form-add-customer' : 'form-edit-customer'"
          type="submit"
        >
