...
const { data, pending } = useAsyncData("admin-operations", async () => {
  const { get } = useApi()
  const [stats, doList] = await Promise.all([
    get<any>("/stats/admin"),
    get<{ items: any[] }>("/delivery-orders", { page: 1, page_size: 10 }),
  ])
  return { stats, doList: doList.items || [] }
...
<template>
  <div class="space-y-6 p-6">
    <h1 class="text-3xl font-bold">Operations — Rekap</h1>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <UCard>
          <template #header><p class="text-sm font-medium">Tren Delivery Order</p></template>
          <AdminBarChart
        ...
        <UCard>
          <template #header><p class="text-sm font-medium">Status Delivery Order</p></template>
          <AdminPieChart
      ...
      <UCard>
        <template #header><p class="text-sm font-medium">Delivery Order Terbaru</p></template>
