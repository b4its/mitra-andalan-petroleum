...
const invTrendData = computed(
  () => data.value?.stats?.trends?.map((t: any) => t.invoices) || [],
);
const invDistLabels = computed(
  () =>
    data.value?.stats?.distributions?.invoices?.map((d: any) => d.label) || [],
);
const invDistValues = computed(
  () =>
    data.value?.stats?.distributions?.invoices?.map((d: any) => d.value) || [],
);
