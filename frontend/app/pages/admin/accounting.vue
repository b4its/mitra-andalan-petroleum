...
// ── Data fetch ────────────────────────────────────────────────
const { data, pending, refresh } = await useAsyncData('admin-accounting', async () => {
  const [summary, journalResult, trial, accounts] = await Promise.all([
    get<AccountingSummary>('/accounting/summary'),
    get<{ items: AccountingJournal[] }>('/accounting/journal', { page: 1, page_size: 100 }),
...
const countCards = computed(() => [
  { title: 'Jumlah Jurnal', value: data.value?.summary?.journal_count ?? 0, icon: 'i-lucide-book-open' },
  { title: 'Jumlah Akun', value: data.value?.summary?.account_count ?? 0, icon: 'i-lucide-list-tree' },
  { title: 'Pemasukan', value: data.value?.summary?.income_count ?? 0, icon: 'i-lucide-arrow-down-to-line' },
  { title: 'Pengeluaran', value: data.value?.summary?.expense_count ?? 0, icon: 'i-lucide-arrow-up-from-line' }
])
...
const journalFiltered = computed(() => {
  ...
  return list.filter(j =>
    ...
    j.lines.some(line => `${line.account_code} ${line.account_name}`.toLowerCase().includes(q))
  )
})
...
const trialFiltered = computed(() => {
  ...
  const list: any[] = data.value?.trial?.rows || []
  if (!q) return list
...
// ── Export ───────────────────────────────────────────────────
const journalExportColumns: ExportColumn[] = [
  ...
  { header: 'Referensi', accessor: (row: AccountingJournal) => row.reference ?? '-' },
  { header: 'Nominal', accessor: (row: AccountingJournal) => totalDebit(row) },
  { header: 'Akun', accessor: (row: AccountingJournal) => row.lines.map(l => l.account_code).join(', ') }
]

const trialExportColumns: ExportColumn[] = [
  { header: 'Kode', accessor: (row: any) => row.account_code },
  { header: 'Nama Akun', accessor: (row: any) => row.account_name },
  { header: 'Tipe', accessor: (row: any) => typeLabels[row.account_type] ?? row.account_type },
  { header: 'Debit', accessor: (row: any) => row.debit },
  { header: 'Kredit', accessor: (row: any) => row.credit }
]

function onJournalExport(format: 'excel' | 'pdf' | 'csv') {
  const filename = `rekap-jurnal-${new Date().toISOString().slice(0, 10)}`
  if (format === 'excel') toExcel(filename, 'Jurnal Umum', journalExportColumns, journalFiltered.value)
  else if (format === 'pdf') toPDF(filename, 'Jurnal Umum', journalExportColumns, journalFiltered.value, { subtitle: 'Catatan transaksi keuangan (debit & kredit)' })
...
function onTrialExport(format: 'excel' | 'pdf' | 'csv') {
  ...
  if (format === 'excel') toExcel(filename, 'Neraca Saldo', trialExportColumns, trialFiltered.value)
  else if (format === 'pdf') toPDF(filename, 'Neraca Saldo', trialExportColumns, trialFiltered.value, { subtitle: 'Saldo debit/credit seluruh akun aktif' })
...
const journalColumns: TableColumn<AccountingJournal>[] = [
  { accessorKey: 'entry_number', header: 'Nomor Jurnal' },
  {
    ...
    header: 'Tanggal',
    cell: ({ row }) => row.getValue('entry_date') ? formatDate(row.getValue('entry_date')) : '-'
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex flex-col gap-0.5 min-w-0' }, [
        ...
        h('span', { class: 'text-xs text-muted' }, row.original.lines.map(l => l.account_code).join(', '))
      ])
    }
  },
  {
  ...
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: ({ row }) => h('span', { class: 'text-xs text-muted capitalize' }, String(row.getValue('status') ?? 'posted'))
  }
]

const trialColumns: TableColumn<any>[] = [
  { accessorKey: 'account_code', header: 'Kode' },
  { accessorKey: 'account_name', header: 'Nama Akun' },
  {
    accessorKey: 'account_type',
    header: 'Tipe',
    cell: ({ row }) => h('span', { class: 'text-xs text-muted' }, typeLabels[row.getValue('account_type')] ?? row.getValue('account_type'))
  },
  {
    accessorKey: 'debit',
    header: 'Debit',
...
const exportItems = (onSelect: (format: 'excel' | 'pdf' | 'csv') => void) => [
  ...
  { type: 'separator' },
  { label: 'Export to Excel', icon: 'i-lucide-file-spreadsheet', onSelect: () => onSelect('excel') },
  { label: 'Export to PDF', icon: 'i-lucide-file-text', onSelect: () => onSelect('pdf') },
  { label: 'Export to CSV', icon: 'i-lucide-file-down', onSelect: () => onSelect('csv') }
]
</script>
    ...
    <template #body>
      <div class="space-y-6 p-4 lg:p-6">
        ...
        <template v-else>
          <!-- Stat cards -->
          <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <UCard v-for="card in valueCards" :key="card.title" variant="subtle">
              <template #leading>
                <UIcon :name="card.icon" class="size-5 text-primary" />
              </template>
              <template #title>{{ card.title }}</template>
              <p class="text-2xl font-semibold tabular-nums">{{ formatCurrency(card.value) }}</p>
          ...
          <div class="grid gap-3 sm:grid-cols-4">
            <UCard v-for="c in countCards" :key="c.title" variant="subtle" class="text-center">
              <UIcon :name="c.icon" class="size-5 text-primary mx-auto mb-1" />
              <p class="text-2xl font-semibold tabular-nums">{{ c.value }}</p>
              <p class="text-xs text-muted mt-1">{{ c.title }}</p>
            </UCard>
          </div>
          ...
          <div class="grid gap-6 lg:grid-cols-2">
            <UCard class="lg:col-span-1">
              ...
              <AdminBarChart
                ...
                :datasets="[{ label: 'Nominal', data: incomeExpenseValues, backgroundColor: ['rgba(16,185,129,0.8)', 'rgba(239,68,68,0.8)'] }]"
              />
            ...
            <UCard>
              <template #header>
                <div class="flex items-center justify-between">
                  ...
                  <p class="text-xs text-muted">{{ (data?.accounts || []).length }} akun</p>
                </div>
              </template>
              <AdminPieChart
          ...
          <UCard>
            ...
            <UEmpty v-if="!journalPaged.length" icon="i-lucide-file-search" title="Tidak ada jurnal" />
            <div v-if="journalFiltered.length > PAGE_SIZE" class="flex items-center justify-between border-t border-default pt-3 px-2 mt-2">
              <p class="text-xs text-muted">{{ journalFiltered.length }} total</p>
              <UPagination v-model:page="journalPage" :total="journalFiltered.length" :items-per-page="PAGE_SIZE" />
            </div>
          </UCard>
          ...
          <UCard>
            ...
            <UEmpty v-if="!trialFiltered.length" icon="i-lucide-file-search" title="Neraca saldo kosong" />

            <!-- Total debit/credit -->
            <div v-if="data?.trial" class="flex items-center justify-end gap-6 border-t border-default pt-3 px-2 mt-2 tabular-nums">
              <div class="flex items-center gap-2 text-sm">
                <span class="text-muted">Total Debit</span>
                <span class="font-semibold text-highlighted">{{ formatCurrency(data.trial.total_debit) }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm">
                <span class="text-muted">Total Kredit</span>
                <span class="font-semibold text-highlighted">{{ formatCurrency(data.trial.total_credit) }}</span>
              </div>
            </div>
          </UCard>
        </template>
