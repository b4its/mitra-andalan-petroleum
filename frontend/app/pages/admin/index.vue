...
const mappedAnalytics = ref<MetricCard[]>(
  [
    data.value.metrics.find((d) => d.key === "outstanding_value"),
    data.value.metrics.find((d) => d.key === "overdue_value"),
    data.value.metrics.find((d) => d.key === "invoice_value"),
    data.value.metrics.find((d) => d.key === "paid_value"),
    data.value.metrics.find((d) => d.key === "sales_value"),
    data.value.metrics.find((d) => d.key === "fuel_volume"),
    data.value.metrics.find((d) => d.key === "delivery_orders"),
    data.value.metrics.find((d) => d.key === "offering_letters"),
    data.value.metrics.find((d) => d.key === "customer_purchase_orders"),
    data.value.metrics.find((d) => d.key === "supplier_purchase_orders"),
    data.value.metrics.find((d) => d.key === "customers"),
    data.value.metrics.find((d) => d.key === "suppliers"),
    data.value.metrics.find((d) => d.key === "unread_notifications"),
    data.value.metrics.find((d) => d.key === "users"),
    data.value.metrics.find((d) => d.key === "uploads"),
...
function distributionChart(key: string) {
  ...
  return {
    ...
    colors: dist.map((_, i) => pieColors[i % pieColors.length] as string),
  };
}

function onDistributionSegmentClick(
  _key: string,
  payload: {
    label: string;
    value: number;
...
const links = [
  [
    {
      label: "Rekap Keseluruhan Sistem",
      to: `#overallSystemSection`,
    },
    {
      label: "Rekap Accounting",
      to: `#accountingSection`,
    },
    {
      label: "Rekap Data Grafik",
      to: `#chartsSection`,
    },
  ],
  ...
  <UDashboardPanel id="admin-overview">
    ...
    <template #body>
      <div class="space-y-6 p-4 lg:p-6">
        <!-- Header -->
        <div id="overallSystemSection" class="flex justify-between">
          <div>
        ...
        <template v-else>
          ...
          <!-- Akuntansi -->
          <!-- Header -->
          <div id="accountingSection" class="flex justify-between">
            <div>
              <h1 class="text-2xl font-semibold">Rekap Data Accounting</h1>
            </div>
          </div>
          <div v-if="accounting" class="space-y-6">
            ...
            <div class="grid gap-3 sm:grid-cols-4">
              ...
              >
                <UIcon
                  :name="c.icon"
                  class="size-5 text-primary mx-auto mb-1"
                />
                <p class="text-2xl font-semibold tabular-nums">
                  {{ c.value }}
                </p>
            ...
            <UCard>
              <template #header>
                <div class="flex items-center justify-between">
                  <p class="font-medium">Jurnal Terbaru</p>
                  <UButton
          ...
          <div id="chartsSection">
            ...
            <div class="mt-6 grid gap-4 lg:grid-cols-2">
              ...
              <UCard v-if="monitoringChart" class="lg:col-span-2">
                <template #header>
                  <p class="font-medium">
                    Monitoring per Bulan ({{ new Date().getFullYear() }})
                  </p>
                </template>
                <AdminBarChart
