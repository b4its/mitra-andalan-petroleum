...
const { data: dos, refresh } = await useAsyncData(
  ...
  async () => {
    ...
    const res = await get<{ items: DeliveryOrderRow[] }>('/delivery-orders', params)
    return res.items || []
  },
...
function statusBadge(done: boolean, label: string, at: string | null | undefined) {
...
async function submitEdit() {
  ...
  try {
    ...
    toast.add({
      title: 'Gagal',
      description: err instanceof Error ? err.message : 'Gagal memperbarui delivery order',
...
async function confirmDelete() {
  ...
  try {
    ...
    toast.add({
      title: 'Gagal',
      description: err instanceof Error ? err.message : 'Gagal menghapus delivery order',
      icon: 'i-lucide-alert-triangle',
      color: 'error'
    })
  } finally {
...
const columns: TableColumn<DeliveryOrderRow>[] = [
  ...
  },
  {
    ...
    header: 'Status',
    cell: ({ row }) =>
      h(UBadge, { variant: 'subtle', color: statusColor[row.original.status] ?? 'neutral', class: 'text-xs' }, () => statusLabel[row.original.status] ?? row.original.status)
  },
  {
    accessorKey: 'status_rilis_dana',
    header: 'Rilis Dana',
    cell: ({ row }) => statusBadge(row.original.status_rilis_dana, 'Dirilis', row.original.rilis_dana_at)
  },
  {
    accessorKey: 'status_ready_order',
    header: 'Siap Kirim',
    cell: ({ row }) => statusBadge(row.original.status_ready_order, 'Siap', row.original.ready_order_at)
  },
  {
    accessorKey: 'status_selesai_dikirim',
    header: 'Selesai Kirim',
    cell: ({ row }) => statusBadge(row.original.status_selesai_dikirim, 'Selesai', row.original.selesai_dikirim_at)
  },
  {
    accessorKey: 'status_lunas_ongkir',
    header: 'Lunas Ongkir',
    cell: ({ row }) => statusBadge(row.original.status_lunas_ongkir, 'Lunas', row.original.lunas_ongkir_at)
  },
          ...
          >
            <template #actions-cell="{ row }">
              <div class="flex flex-wrap items-center gap-1.5">
                ...
                <UButton
                  v-if="row.original.status_ready_order && !row.original.status_lunas_ongkir"
                  size="xs"
                  color="warning"
                  variant="soft"
                  icon="i-lucide-truck"
    ...
    <template #body>
      <div class="space-y-4">
        ...
        <UFormField label="Customer" required>
          <USelect v-model="form.customer_id" :items="customerItems" value-key="value" />
        </UFormField>
        <div class="grid grid-cols-2 gap-3">
          ...
          <UFormField label="Transportir">
            <UInput v-model="form.transport_name" placeholder="Nama transportir" />
          </UFormField>
        </div>
        <div class="grid grid-cols-2 gap-3">
  ...
  <UModal v-model:open="deleteOpen" :ui="{ content: 'max-w-md' }">
    ...
    <template #body>
      <p class="text-sm text-muted">
        ...
        <span class="font-semibold text-highlighted">{{ deleteTarget?.do_number }}</span>?
      </p>
  ...
  <UModal v-model:open="rilisDanaOpen" :ui="{ content: 'max-w-md' }">
    ...
    <template #body>
      <p class="text-sm text-muted">
        ...
        <span class="font-semibold text-highlighted">{{ rilisDanaTarget?.do_number }}</span>?
      </p>
      <p class="mt-2 text-xs text-dimmed">
  ...
  <UModal v-model:open="lunasOngkirOpen" :ui="{ content: 'max-w-md' }">
    ...
    <template #body>
      <p class="text-sm text-muted">
        ...
        untuk Delivery Order
        <span class="font-semibold text-highlighted">{{ lunasOngkirTarget?.do_number }}</span>?
      </p>
      <p class="mt-2 text-xs text-dimmed">
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <UButton color="neutral" variant="ghost" @click="lunasOngkirOpen = false">
          Batal
        </UButton>
        <UButton
          color="warning"
