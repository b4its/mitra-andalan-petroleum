...
export function useOfferingLetterPdf() {
  ...
  async function buildOfferingLetterPdf(
    ...
    const signatureCaption =
      extra?.signatureCaption ||
      details?.offeror?.name ||
      user.value?.name ||
      "";
    const signatureBarcode =
      extra?.signatureBarcode ||
      buildSignatureBarcodeData(
        details?.offeror?.name || user.value?.name || "",
      );
    ...
    const paymentMethodLabel = (() => {
      ...
      if (method === "cash") {
        const cashLabel =
          cashMethod === "cash_before_delivery"
            ? "Tunai Sebelum Pengiriman"
            : "Tunai Setelah Pengiriman";
    ...
    const baseWithPpkb =
      details.fuelPrices.basePrice + details.fuelPrices.sellingPrice.ppkb;
    ...
    return await pdfMake
      .createPdf({
        ...
        content: [
          ...
          {
            text: [
              "Mohon Purchase Order (PO) dapat dikirimkan pada periode ",
              {
